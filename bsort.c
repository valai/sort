...
int mysort(char *string_array, int **order, int size) {
  ...
  //input your code here...
	...
	for(i = 0; i < size; i++){
		t_order[i] = (int*)malloc(sizeof(int)*2);
	}
...
void Msort(char *Data, int **order, int **t_order, int left, int right, int size){
	...
	while(i <= mid || j <= right){
		if(j > right || (i <= mid && strncmp(Data + t_order[i][0], Data + t_order[j][0], size) <= 0)){
			order[k][0] = t_order[i][0];
			order[k][1] = t_order[i][1];
			i++;
		}else{
			order[k][0] = t_order[j][0];
